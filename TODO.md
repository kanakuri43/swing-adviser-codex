...
- [x] **4.1.6 一部利確候補数量の計算を実装する**（`依存: 4.1.4`。現在数量の50%を売買単位で切り下げ、決済後にも1売買単位以上残せる場合だけ `Candidate`、不可能なら `NotFeasible` とする。全決済候補への暗黙変換や保有数量の自動変更を禁止するテストを追加する）
  - **実装結果（2026-08-28）**: Domain層に純粋計算の`LotPartialExitQuantityCalculator`を追加した。企業アクション後の端株を保持できるdecimalのlot現在数量へ凍結済み`partialTakeProfitFraction`を掛け、売買単位で切り下げた整数株を計算する。候補が正で決済後にも同じlotへ最低1売買単位が残る場合だけ、lot IDに結び付いた`Candidate`、候補数量、残数量、実効割合を返す。分割不能時は数量なしの`NotFeasible`とし、全決済への丸め、別lotへの暗黙配分、約定・allocation・保有数量の生成変更をAPI上行わない。50%の通常・切り下げ・最低残数量境界、端株、分割不能、99%でも最低残数量を割る場合、default/非正入力、表現範囲超過をDomainテストで確認した。
- [x] **4.1.7 利用者登録済み部分決済後の建値移動を実装する**（`依存: 4.1.3, 4.1.6`。有効な部分決済約定と明示lot配分revisionがある場合だけ、Longは `max(従来stop, entry_basis)`、Shortは `min(...)` のplan revisionを追記する。価格到達だけでは追記せず、stopを不利な方向へ緩めず、旧planを上書きしないことをテストする）
  - **実装結果（2026-08-28）**: Domain層に`PartialExitBreakevenPlanFactory`（`partial-exit-breakeven-plan-factory-v1`）を追加し、利用者確認済みCloseのcurrent effective revision、同一lotへ明示されたexact effective allocation、決済直前の正の残数量、active risk basis/planのdirect predecessorを照合して、Longは`max(従来stop, 現在基準entry)`、Shortは`min(...)`の`PartialExitBreakeven` revisionを生成する。手動Close登録transactionへ接続し、部分決済の場合だけexecution/allocationと同時に新planを追記し、全決済では追記しない。新planはClose約定時刻をeffective時刻とし、logical execution IDとexact allocation revision IDをtriggerに保持し、targetと旧plan rowを変更しない。Long/Short、企業アクション後の現在基準entry、既に有利なstop、全決済/過剰配分、別lot/basis、stale Close revision、時刻・direct predecessor不正、価格到達だけでは非発火、Repositoryでの同一transaction追記・不正遷移時の全体rollback・旧row不変をテストした。
- [x] **4.1.8 価格損益・信用コスト・コスト/Rのlot別集計を実装する**（`依存: 4.1.2`。Long/Shortの価格損益、確定コスト控除後損益、見積込みネット損益、コスト/Rを算出する。EstimateとConfirmedの二重計上を避け、既知0と未公表・取得失敗・不明を区別し、欠損を0へ変換しない）
  - **実装結果（2026-08-28）**: Domain層に純粋計算の`LotProfitAndLossCalculator`（`lot-profit-and-loss-v1`）を追加した。risk basisと同一の価格単位・現在数量からLongは`(current-entry)*数量`、Shortは`(entry-current)*数量`の価格損益を求め、Chargeを正・Creditを負とするnet costを控除して確定コスト控除後損益、見積込みネット損益、`net cost/(1R*数量)`をlot単位で返す。logical cost itemごとに解決済みConfirmedをEstimateより優先して1件だけ選び、未解決ConfirmedはEstimateへフォールバックしつつ両leafの状態とIDを証跡へ残す。空台帳、確定leaf不足、`Unpublished`/`FetchFailed`/`Unknown`、不正default、通貨・株式単位不一致を0へ変換せず、`KnownZero`/`NotOccurred`/`NotApplicable`を区別する。Long/Short、確定による見積置換、未解決確定から見積へのfallback、Charge/Credit、複合欠損、重複occurrence、単位不正、overflowをDomainテスト17件で確認し、約定・lot・コスト台帳は変更しない。
- [x] **4.1.9 再評価用point-in-timeポジションprojectionと入力manifest生成を実装する**（`依存: 4.1.3, 4.1.7, 4.1.8`。約定、lot配分、企業アクション調整、契約、risk basis/plan、コストのexact leaf IDをcutoff時点で選択して正規化JSON/hashを生成する。後日訂正後の再構築、同一position/lot graph、要照合・未対応actionのfail-closedをRepositoryテストで確認する）
  - **実装結果（2026-08-28）**: Infrastructure層に保存を行わない`PositionEvaluationProjectionRepository`（`position-projection-v1`）を追加した。analysis run/market manifestのcutoffから評価日価格revisionを検証し、position state、利用者確認済み約定、明示lot配分、企業アクション調整、契約、risk basis/plan、Estimate/Confirmedコストをlogical chainごとのexact as-of leafとして選択する。部分決済と分割・併合はJSTの実効順およびaction ordinalで投影し、現在数量・取得単価・固定ATR・1R・stop/targetと共通のcurrent-unit hashを生成する。全決済済みlotは監査graphへ残しつつ評価用risk graphを要求しない。正規化したversion付きID JSONと、position/lot自然キー、各revision ID/content hash、価格、position state、企業アクションapplication内容を含むcanonical JSON/SHA-256を生成し、4.1.10の原子的保存へ渡せるdraftにした。revision分岐・cross-position/lot参照・Close数量と配分不一致・企業アクション倍率/plan trigger不整合・unit provenance不正は拒否し、PIT未検証、要照合、未対応action、調整欠落は`Hold`へ変換せずfail-closed状態を返す。後日訂正後の旧cutoff再構築、明示配分、部分決済→分割、全決済lot、canonical hash決定性、PIT/要照合/未対応actionをSQLite Repositoryテスト11件で確認した。
- [x] **4.1.10 評価manifestと評価結果の原子的な永続化を実装する**（`依存: 4.1.4〜4.1.9`。`position_evaluation_input_manifests` と `position_evaluations` を同一transactionでappend-only保存し、lot別根拠を残す。hash、同一run/position重複拒否、読戻し、同一銘柄の複数positionが独立することを結合テストで確認する）
  - **実装結果（2026-08-29）**: Infrastructure層に`PositionEvaluationRepository`を追加し、4.1.9のprojectionを同一cutoffから再構築してcanonical hash、派生数量、current-barのstop/target優先、価格損益、Estimate/Confirmedの選択、観測未作成cost itemの欠損伝播、cost/R、一部利確数量とPIT時点の売買単位を再検証したうえで、入力manifestと評価結果を1 transaction・1 SaveChangesでappend-only保存する。理由codeとlot別のbasis/plan、判定、部分利確、構造化根拠はversion付きcanonical JSONへ正規化し、読戻し時にもexact graph/hashと全集計を再検証する。同一run/positionは重複拒否し、同一銘柄の別positionは独立して保存する。`evaluation_outcome`、nullable decision/数量、fail-closed CHECKを追加するmigrationは既存行を`Evaluated`として保全し、table再構築とimmutable trigger復元を原子的に行う。正常保存/読戻し、hash・派生projection改ざん、判定/P&L矛盾、観測ゼロcost、PIT失敗、評価INSERT失敗時rollback、重複、同一銘柄複数position、売買単位、trigger、upgrade/downgradeをSQLite Repositoryテスト13件で確認した。
- [x] **4.1.11 保有ポジション再評価のApplicationユースケースを実装する**（`依存: 4.1.5〜4.1.10`。Open positionを決定的順序で処理し、position単位の失敗継続、進捗、キャンセル、run status集計を実装する。全成功・一部失敗・全失敗・同一銘柄複数positionをテストし、`trade_executions`を生成しないことを確認する）
  - **実装結果（2026-08-29）**: Application層に`PositionReevaluationService`と`IPositionReevaluationRepository`を追加した。Open positionの取得と「position評価＋append-only保存」だけをportとして公開し、約定・lot allocation・risk-planの変更経路を契約から除外した。対象はinstrument UUID→position UUIDの決定的順で逐次処理し、同一銘柄でもposition IDが異なれば独立して処理する。position単位の予期しない失敗は`POSITION_REEVALUATION_FAILED`として結果へ隔離して後続を継続し、`Succeeded`/`PartiallySucceeded`/`Failed`を集計する。キャンセルは次のposition開始前に伝播し、進捗observerの例外は既に完了した評価を無効化しない。全成功・一部失敗・全失敗・同一銘柄複数position・キャンセル・重複target拒否・progress observer失敗をApplicationテスト7件で確認し、評価保存テストでは`trade_executions`件数が不変であることも回帰確認した。
- [x] **4.1.12 保有画面への再評価結果接続と安全性の結合確認を行う**（`依存: 4.1.11`。最新評価、損切・利確候補、Hold理由、価格/コスト損益、要照合状態を既存の読取経路へ反映する。利用者が明示登録するまで数量・risk plan・約定履歴が変化しない回帰テストと実画面確認を行う）
  - **実装結果（2026-08-29）**: `TradingWorkspace`の読取契約へ再評価outcome・作成日時・鮮度・確定コスト控除後損益・ネット参考損益・cost/R・一部利確候補を追加し、保有画面に状態と各値を表示した。評価が存在する場合は入力manifestが固定した価格revisionだけを参照し、現在数量またはactive risk-plan leafがmanifestと異なる（あるいは価格/manifestが不正な）場合は`Stale`な参考表示へ落とし、決済判定をnull化して「保有から登録」を無効化する。`ReconciliationRequired` outcomeはpositionを要照合へ反映し、fail-closedを`Hold`へ変換しない。SQLite結合テストでは固定価格・損益欄・読取前後のposition/execution/allocation/risk plan/evaluation graph不変、利用者確認済み部分決済後のstale化、要照合outcomeを確認した。WPFテストでstale/fail-closed時の登録導線無効化と欠損値の非ゼロ表示を確認し、隔離した開発データ起動のUI Automationで保有画面の23列と追加6列を実画面確認した。
