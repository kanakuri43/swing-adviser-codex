...
- [ ] 上記が揃った時点で Serena MCP の onboarding を実行する
  - 理由: Serena は language server 経由のシンボル探索（find_symbol/find_referencing_symbols 等）が主機能のため、コードが存在しない現時点では解析対象がなく onboarding の効果が薄い。.sln + 最低限の C# プロジェクト構造ができた段階で行うのが適切。
...
現行の [`data-sources.md`](docs/data-sources.md) 「テクニカル分析は原則 adjclose を基準に行う」が、以下3点と衝突している。

- [ ] **Look-ahead bias 規則との矛盾を解消する**
...
- [ ] **保有中の企業アクションに対するポジション調整仕様を追加する**（現状まったく規定がない）
- [ ] 対処方針を決めてドキュメントへ反映する
...
- [ ] **EMA のシード方法（SMA シード / 単純初値）と計算開始日の固定ルールを仕様化する**
...
- [ ] **建玉期限を Positions に追加する**（制度信用6ヶ月。塩漬けポジションの期限管理）
- [ ] **Short 側の損切を非対称にするか判断する**
  - エントリー条件は「Short は全条件一致必須」と非対称にした一方、損切は Long/Short とも ATR×3 で対称。踏み上げリスクの非対称性を理由にエントリーを厳格化したのなら、リスク管理側も揃えるべきか検討する。

### 2-4. 未定義で実装時に詰まる箇所
- [ ] **「一部利確」の割合を決める**（何割決済するか）
- [ ] **一部利確後の残ポジションの損切を建値へ移すか、元のまま維持するかを決める**（成績への影響大）
- [ ] **ATR の基準日を決める**（約定日 / 分析日。エントリー時に固定するか毎日再計算するか）
  - 15:30 以降に更新 → 翌日寄りで約定、というフロー上、約定日と分析日は必ずズレる。
- [ ] **EMA200 を満たさない銘柄（上場200日未満）の扱いを決める**（除外 / 指標の部分適用）
- [ ] **AI チェックを日次フローのどこに置くか決める**
